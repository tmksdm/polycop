...
def _configure_plain_logger() -> None:
    """
    Перенастраивает logger под разовый режим без Rich Live.

    Та же логика, что и в начале run_leaderboard_mode:
    при импорте main.py уже сработал setup_logger(), поэтому
    сначала убираем все старые трубы, ставим одну свою, и отключаем
    propagate, чтобы не было дублей сообщений.

    Вынесли в отдельную функцию, чтобы и лидерборд, и clob-check
    могли её переиспользовать без копипасты.
    """
    logger.handlers.clear()
